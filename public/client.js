...
// --- INÍCIO: bloco compat (SUBSTITUI O BLOCO ORIGINAL) ---
function el(id){ return document.getElementById(id); }

// Mapeamento compatível: prioriza elementos do modal do chat (wt*), cai para ids originais
const roomIdInput = el('wtRoom') || el('roomId');
const createBtn = el('createBtn') || el('wtCreate') || null;
const joinBtn = el('joinBtn') || el('wtJoin') || null;
const fileInput = el('fileInput') || el('wtFileInput') || null;
const loadBtn = el('loadBtn') || el('wtLoadUrl') || null;
const startStreamBtn = el('startStreamBtn') || null;
const playBtn = el('playBtn') || el('wtPlay') || null;
const pauseBtn = el('pauseBtn') || el('wtPause') || null;
const unmuteBtn = el('unmuteBtn') || null;
const player = el('player') || el('wtVideo'); // prefer wtVideo when inside chat modal
const statusEl = el('status') || el('wtStatus') || (function(){
  const d = document.createElement('div');
  d.style.display='none';
  document.body.appendChild(d);
  return d;
})();
const logEl = el('log') || (function(){
  let d = el('wtLog');
  if(!d){
...
if (!player) {
  console.warn('Watch: nenhum elemento <video> encontrado (esperado player ou wtVideo). A funcionalidade ficará limitada.');
}
// --- FIM: bloco compat ---
...
async function createPC(peerId, makeOffer=false, remoteSdp=null) {
  ...
  if (!isHost) {
    ...
    pc.ondatachannel = (ev) => { dc = ev.channel; dc.onmessage = ()=>{}; };
  } else {
...
// --- API pública (para o adapter) ---
window.__WATCH_CLIENT_LOADED = true;

// exponha funções se já existirem no script
window.joinRoom = window.joinRoom || (typeof joinRoom === 'function' ? joinRoom : function(roomId){ console.warn('joinRoom não implementado nativamente'); });
window.connectWS = window.connectWS || (typeof connectWS === 'function' ? connectWS : null);
window.sendWS = window.sendWS || (typeof sendWS === 'function' ? sendWS : null);

// controles de alto nível (opcional — implemente em client.js se quiser comportamento custom)
window.enableWatchSync = window.enableWatchSync || function(){ console.log('enableWatchSync(): adicione implementação no client.js se quiser comportamento custom'); };
window.disableWatchSync = window.disableWatchSync || function(){ console.log('disableWatchSync(): adicione implementação no client.js se quiser comportamento custom'); };
